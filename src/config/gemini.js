const simulatedResponses = {
  creator: [
    ...
    "**Irsyad Ibadurrahman is the developer of this Gemini clone.** *He built this project* using React.js with a focus on functionality and user experience. *The application includes* conversation history, responsive layout, and smooth interaction. *It shows* how modern web development can create an AI-powered chat interface!",
  ],
...
function detectPromptType(prompt) {
  ...
  if (
    lowerPrompt.includes("who made this") ||
    lowerPrompt.includes("who created this") ||
    lowerPrompt.includes("who built this") ||
    lowerPrompt.includes("creator") ||
    lowerPrompt.includes("developer") ||
    lowerPrompt.includes("author")
  ) {
    return "creator";
  } else if (
...
async function runChat(prompt) {
  ...
  try {
    ...
    if (promptType !== "greetings") {
      response += `\n\n*Regarding your specific question about "${prompt.slice(
        0,
        50
      )}${
        prompt.length > 50 ? "..." : ""
      }"*, I hope this helps! Feel free to ask if you need more clarification or have follow-up questions.`;
    }
